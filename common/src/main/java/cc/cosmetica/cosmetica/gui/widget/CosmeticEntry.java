	...
	public static void populateEntryList(final List<CosmeticEntry> entryList, Cosmetics cosmetics, Type type) {
		...
		if (cosmetics.getCloak().isPresent()) {
			...
			entryList.add(new CosmeticEntry(
					...
					cloak.getId(),
					cloak.getName(),
					cloak.getCreator().isPresent() ? cloak.getCreator().get().name() : "Could not load creator",
		...
		if (showSeparateElytra && cosmetics.getElytra().isPresent()) {
			...
			entryList.add(new CosmeticEntry(
					...
					elytra.getId(),
					elytra.getName(),
					elytra.getCreator().isPresent() ? elytra.getCreator().get().name() : "Could not load creator",
		...
		for (Accessory accessory : cosmetics.getAccessories()) {
			...
			CachedImage thumbnail = accessory.getThumbnail().isEmpty() ?
			...
			entryList.add(entry = new CosmeticEntry(
					...
					accessory.getId(),
					accessory.getName(),
					accessory.getCreator().isPresent() ? accessory.getCreator().get().name() : "Could not load creator",
					type,
